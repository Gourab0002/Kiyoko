...
export async function fileInfoScraper(c: Context, url: string) {
  ...
  if (response.status === 200) {
    ...
    const torrentData: Models.Torrent = {
      ...
      file:
        Constants.SukebeiBaseUrl +
        (container.find("div.panel-footer a").attr("href") ?? ""),
      link: `${Constants.SukebeiBaseUrl}/view/${fileId}`,
      id: fileId,
...
export async function scrapeSukebei(c: Context, url: string) {
  const response = await fetch(url);
  ...
  if (response.status === 200) {
    ...
    table.find("tr").each((_, selection) => {
      ...
      const torrent: Models.Torrent = {
        ...
        link: Constants.SukebeiBaseUrl + torrentPath,
        file: Constants.SukebeiBaseUrl + filePath,
