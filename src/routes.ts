...
export class Handlers {
  static Ping = function (c: Context) {
    ...
    return c.text("Kiyoko — Sukebei Nyaa API v1 // Alive");
  };
  ...
  static Categories = function (c: Context) {
    Utils.setCache(c, 3600);
    return c.json({ categories: Constants.SukebeiEndpoints });
  };

  static Search = async function (c: Context) {
    try {
      const path = `/?${listingQuery(c)}`;
      return await Scrapers.scrapeSukebei(c, path, { envelope: true });
    } catch (error) {
  ...
  static GetUserUploads = async function (c: Context) {
    try {
      ...
      return await Scrapers.scrapeSukebei(c, path, { username });
    } catch (error) {
      return Utils.jsonErrorFrom(c, error);
    }
  };
  ...
  static GetUserProfile = async function (c: Context) {
    try {
      ...
      const result = await Utils.fetchSukebei(`/user/${encodeURIComponent(username)}`);
  ...
  static GetCategoryTorrents = async function (c: Context) {
    try {
      ...
      return await Scrapers.scrapeSukebei(c, path);
    } catch (error) {
      return Utils.jsonErrorFrom(c, error);
    }
  };
